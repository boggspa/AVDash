...
public struct CloudHardwareDashboardBuilder {
    ...
    public func makeCurrentSnapshotPayload() -> CompanionCurrentSnapshotPayload {
    ...
    public func makeDashboardSnapshotPayload(
    ...
    public func makeMinuteTimelinePayload() async -> CompanionTimelinePayload {
    ...
    public func makeHourlyTimelinePayload() async -> CompanionTimelinePayload {
    ...
    public func makeProcessRollupPayload() async -> CompanionProcessRollupPayload {
    ...
    public func makeHardwareEventPayload() async -> CompanionHardwareEventPayload {
    ...
    private func makeLiveSnapshot() -> CompanionLiveSnapshot {
        ...
        let network = CompanionLiveNetworkSnapshot(
            ...
            interfaceName: polling.networkInterfaceSnapshot?.interfaceName,
            localIP: polling.networkInterfaceSnapshot?.ipv4Address,
            subnetMask: nil,
            router: polling.networkInterfaceSnapshot?.routerAddress,
            dnsServers: polling.networkInterfaceSnapshot?.dnsServers ?? [],
            searchDomains: [],
            ethernetSpeed: nil,
            configMethod: nil
        )
    ...
    private func connectionLabel(from snapshot: NetworkInterfaceSnapshot?) -> String {
        ...
        if let ipv4Address = snapshot.ipv4Address, !ipv4Address.isEmpty {
            return ipv4Address
        }
        return "Connected"
    }
