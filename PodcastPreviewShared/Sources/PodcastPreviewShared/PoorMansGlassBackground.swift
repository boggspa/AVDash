...
public struct PoorMansGlassBackground: NSViewRepresentable {
    public let style: AppChromeStyle
    public let cornerRadius: CGFloat
    public let reduceBlur: Bool
    public let tuning: PoorMansGlassTuning
    public let appearance: ThemeAppearanceOverride

    public init(style: AppChromeStyle, cornerRadius: CGFloat, reduceBlur: Bool = false, tuning: PoorMansGlassTuning = .neutral, appearance: ThemeAppearanceOverride = .system) {
        self.style = style
    ...
    public func makeNSView(context: Context) -> NSVisualEffectView {
        ...
        // Layered “glass” accents
        installOrUpdateLayers(in: vfx)
        return vfx
    }

    public func updateNSView(_ vfx: NSVisualEffectView, context: Context) {
        vfx.material = materialForStyle
        vfx.state = .active
        vfx.blendingMode = reduceBlur ? .behindWindow : .withinWindow
        vfx.isEmphasized = false
    ...
    private var materialForStyle: NSVisualEffectView.Material {
        ...
        // These choices are conservative for Big Sur, but still give real blur/vibrancy.
        switch style {
        case .panel:
    ...
    private var tintColor: NSColor {
        ...
        switch style {
        case .panel:
            return NSColor.white.withAlphaComponent(scaledAlpha(reduceBlur ? 0.14 : 0.08))
        case .header:
            return NSColor.white.withAlphaComponent(scaledAlpha(reduceBlur ? 0.11 : 0.06))
        case .hud:
            return NSColor(
                calibratedWhite: 0.05,
                alpha: scaledAlpha(reduceBlur ? 0.12 : 0.055)
            )
        }
    }
    ...
    private var borderColor: NSColor {
        if appearance == .midnight {
            return NSColor.white.withAlphaComponent(scaledAlpha(0.12, floor: 0.35))
        }
        switch style {
        case .panel:
    ...
    private func scaledRimAlpha(_ base: CGFloat, floor: CGFloat = 0.0) -> CGFloat {
    ...
    private func installOrUpdateLayers(in vfx: NSVisualEffectView) {
        ...
        let isPillLike = cornerRadius >= 24
        let pillBoost: CGFloat = isPillLike ? 1.10 : 1.0
        ...
        root.shadowRadius = style == .hud ? 24 : (isPillLike ? 20 : 18)
        root.shadowOffset = CGSize(width: 0, height: style == .hud ? -8 : (isPillLike ? -7 : -6))
        ...
        if let g = ambient as? CAGradientLayer {
            g.type = .radial
            g.frame = bounds
            g.startPoint = CGPoint(x: 0.5, y: 0.58)
            g.endPoint = CGPoint(x: 1.0, y: 1.0)
            let centerAlpha: CGFloat = style == .hud ? (reduceBlur ? 0.13 : 0.19) : (reduceBlur ? 0.09 : 0.13)
            let baseColor = appearance == .midnight ? NSColor(hex: "#1A44AA") : NSColor.white
            g.colors = [
                baseColor.withAlphaComponent(scaledAlpha(centerAlpha * 0.92 * pillBoost)).cgColor,
                baseColor.withAlphaComponent(scaledAlpha(centerAlpha * 0.34 * pillBoost)).cgColor,
                NSColor.clear.cgColor
            ]
            g.locations = [0.0, 0.34, 1.0]
        }
        ambient.compositingFilter = appearance == .midnight ? "screenBlendMode" : "screenBlendMode"
        ...
        if let g = chroma as? CAGradientLayer {
            ...
            if appearance == .midnight {
                let cool = NSColor(hex: "#1A44AA").withAlphaComponent(scaledChromaAlpha(0.15 * pillBoost)).cgColor
                let neutral = NSColor(hex: "#050812").withAlphaComponent(scaledChromaAlpha(0.12 * pillBoost)).cgColor
                let warm = NSColor(hex: "#010103").withAlphaComponent(scaledChromaAlpha(0.10 * pillBoost)).cgColor
                ...
                let cool = NSColor(
                    ...
                    green: 0.84,
                    blue: 0.98,
                    alpha: scaledChromaAlpha((style == .hud ? 0.095 : 0.055) * pillBoost)
                ).cgColor
                let neutral = NSColor.white.withAlphaComponent(
                ...
                let warm = NSColor(
                    ...
                    green: 0.63,
                    blue: 0.34,
                    alpha: scaledChromaAlpha((style == .hud ? 0.075 : 0.045) * pillBoost)
                ).cgColor
                g.colors = [cool, neutral, warm]
            }
        ...
        chroma.compositingFilter = appearance == .midnight ? "screenBlendMode" : "overlayBlendMode"
        ...
        if let g = specular as? CAGradientLayer {
            ...
            let top = NSColor.white.withAlphaComponent(
                scaledHighlightAlpha(
                    (reduceBlur ? (style == .hud ? 0.10 : 0.12) : (style == .hud ? 0.18 : 0.18)) * pillBoost
                )
            ).cgColor
            let mid = NSColor.white.withAlphaComponent(
        ...
        if let g = caustic as? CAGradientLayer {
            ...
            g.colors = [
                NSColor.white.withAlphaComponent(scaledHighlightAlpha((style == .hud ? 0.18 : 0.10) * pillBoost)).cgColor,
                NSColor.white.withAlphaComponent(scaledHighlightAlpha((style == .hud ? 0.055 : 0.028) * pillBoost)).cgColor,
                NSColor.clear.cgColor
            ]
        ...
        if let g = haze as? CAGradientLayer {
            ...
            g.colors = [
                NSColor.white.withAlphaComponent(scaledHazeAlpha(a * pillBoost)).cgColor,
                NSColor.white.withAlphaComponent(scaledHazeAlpha(a * 0.42)).cgColor,
                NSColor.clear.cgColor
            ]
        ...
        if let g = vignette as? CAGradientLayer {
            ...
            g.colors = [
                NSColor.clear.cgColor,
                NSColor.clear.cgColor,
                NSColor.black.withAlphaComponent(scaledHazeAlpha(style == .hud ? 0.085 : 0.055)).cgColor
            ]
        ...
        noise.opacity = Float(
            scaledAlpha(
                (reduceBlur ? (style == .hud ? 0.028 : 0.03) : (style == .hud ? 0.045 : 0.045)) * (isPillLike ? 0.88 : 1.0)
            )
        )
        ...
        if let s = border as? CAShapeLayer {
            ...
            s.strokeColor = borderColor.withAlphaComponent(isPillLike ? 0.92 : 1.0).cgColor
            s.lineWidth = 1.0
        }
        ...
        if let s = rim as? CAShapeLayer {
            ...
            s.strokeColor = NSColor.white.withAlphaComponent(
                scaledRimAlpha((style == .hud ? 0.09 : 0.055) * pillBoost, floor: 0.55)
            ).cgColor
            s.lineWidth = 1.0
        }
